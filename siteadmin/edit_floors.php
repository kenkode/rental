...
$result = mysql_query("SELECT * FROM floors where fid='".$_REQUEST['id']."'");
...
echo '<h2>Edit Floor Details</h2>' ;   

echo ' <table border="0"> 
<tr><td><p>Floor No:</td><td> <input type="text" name="floor_no" required size="50" maxlength="50" value="'.$row['floor_no'].'" class="textbox"/></p></td></tr>
<tr><td><p style="margin-top:-90px !important">Building</p></td><td><p>
...
if (isset($_POST['submitted'])) {
    ...
    if (empty($errors)) { 
        $query = "UPDATE floors SET floor_no = '$floor_no',building_id = '$building_id' WHERE fid = '".$_REQUEST['id']."'";
        addTable($dbc,$query);
