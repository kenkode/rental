...
$result = mysql_query("SELECT * FROM landlord where ownid='".$_REQUEST['id']."'");
...
echo '<h2>Edit Landlord Details</h2>' ;   
...
if (isset($_POST['submitted'])) {
    ...
    if (empty($errors)) { 
        $query = "UPDATE landlord SET o_name = '$o_name',o_email = '$o_email',o_contact = '$o_contact',o_address = '$o_address',o_nid = '$o_nid',o_password = '$o_password',building_id = '$building_id'
      WHERE ownid = '".$_REQUEST['id']."'";
        addTable($dbc,$query);
		echo '<div class="success"><p class="success">Landlord successfully updated!</p></div>';
    }else {  
