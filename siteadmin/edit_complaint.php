...
<?php 
// Create the form.
$result = mysql_query("SELECT * FROM complaints where complain_id='".$_REQUEST['id']."'");
...
if (isset($_POST['submitted'])) {
	...
	$year = date('Y',strtotime($c_date));
        $query = "UPDATE complaints SET c_title = '$c_title',c_description = '$c_description',c_date = '$c_date',c_month = '$month',c_year = '$year'
      WHERE complain_id = '".$_REQUEST['id']."'";
        addTable($dbc,$query);
		echo '<div class="success"><p class="success">Complaint successfully updated!</p></div>';
    }else {  
